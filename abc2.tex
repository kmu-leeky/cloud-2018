...
In this work, we confirm that the accurate prediction of distributed matrix multiplication is feasible. On top of the finding in this paper, we are actively working on selecting a small number of representative distributed matrix multiplication workloads that still provides high prediction accuracy to decrease the performance profiling overheads in different cloud computing instances. Beside eight features used for modeling task latency on MPC, we plan to add more features of cloud configurations to uniform diverse cloud computing instances features such as clock rate, network bandwidth, memory size. These works can further be improved to predict the latency with a different number of workers and different types of cloud computing instances.
